...
- `circle(cylBody1.startFaces(), E)` sketches a smaller circle of diameter `E` (41mm) on the pipe's far end face
...
- `circle(corner.endFaces(), 15)` sketches a 15mm radius circle on the top face of the corner block, and `cut()` with no depth argument cuts through the entire block
- `circle(corner.endFaces(), 30)` adds a larger 30mm radius circle on the same face, and `cut(10)` cuts only 10mm deep — creating the counterbore recess
