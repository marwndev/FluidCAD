...
- `arc(l1.end(), l2.end(), [leftOffset, C])` connects the two line endpoints with an arc, using `[leftOffset, C]` as the center point — this creates the curved corner of the L
...
- `.hasEdge(edge().line(45))` narrows it to the face that has a 45-degree edge — this is the L-shaped support's footprint, which has the diagonal arc
...
- `rect(15).center('horizontal')` draws a 15mm square centered horizontally at the top of the circle
