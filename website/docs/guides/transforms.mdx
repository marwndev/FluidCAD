...
:::tip[Primitive shortcut]
`cylinder()` and `sphere()` also expose chainable `.translate()`, `.rotate()`, and `.mirror()` methods for placing a primitive in a single expression. See the [Chained Transforms guide](./chained-transforms.mdx).
:::
