...
Most 2D geometries can be used outside a `sketch()` block by passing the plane as the first argument. This is consistent across every 2D shape:

```js
circle("xy", 50)               // circle on the XY plane
rect("front", 100, 60)         // rectangle on the front plane
line("xz", [0, 0], [100, 50])  // line on the XZ plane
slot("xy", 80, 20)             // slot on the XY plane
arc("front", 50, 0, 90)        // arc on the front plane
```
