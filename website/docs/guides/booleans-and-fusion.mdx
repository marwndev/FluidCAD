...
`subtract()` also works inside sketches to cut 2D geometries from each other:
...
| Subtracting a shape from another | `subtract(keep, remove)` (works in 3D and 2D sketches) |
